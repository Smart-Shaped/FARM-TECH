...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            registered_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {registered_group.name}')
            )

            azione_1_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_1_group.name}')
            )

            azione_2_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_2_group.name}')
            )

            azione_3_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_3_group.name}')
            )

            azione_4_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_4_group.name}')
            )

            azione_5_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_5_group.name}')
            )

            azione_6_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_6_group.name}')
            )

            azione_zootecnica_group.permissions.add(perm_view_res, perm_download_res, perm_viewer, perm_inference, perm_request, perm_add_res, perm_uploader)
            self.stdout.write(
                self.style.SUCCESS(f'Added permissions to group: {azione_zootecnica_group.name}')
            )
