...
class KeycloakAuthentication(authentication.BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        try:
            ...
            if not issuer:
                raise exceptions.AuthenticationFailed('Keycloak non configurato')
            ...
            if not username:
                raise exceptions.AuthenticationFailed('Username non presente nel token')
            ...
            if not created:
                ...
                try:
                    ...
                    logger.info(f"Gruppi 'anonymous' e 'registered-members' assegnati all'utente {username}")
                except Exception as e:
                    logger.error(f"Errore nell'assegnazione dei gruppi all'utente {username}: {str(e)}", exc_info=True)
            ...
            raise exceptions.AuthenticationFailed(f'Token non valido: {str(e)}')
        except Exception as e:
            logger.error(f'Errore autenticazione Keycloak: {str(e)}', exc_info=True)
            raise exceptions.AuthenticationFailed(f'Errore autenticazione: {str(e)}')
